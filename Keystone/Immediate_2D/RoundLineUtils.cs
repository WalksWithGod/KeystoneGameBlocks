...
namespace Keystone
{
    public partial class RoundLine
    {
        ...
        /// </summary>
        public float DistanceSquaredPointToVirtualLine(Vector2d p, out Vector2f closestP)
        {
            Vector2f v = P1 - P0; // Vector from line's p0 to p1
            Vector2f w = p - P0; // Vector from line's p0 to p
            ...
            {
                closestP = P0;
                return Vector2f.DistanceSquared(p, P0);
            }
            ...
            {
                closestP = P1;
                return Vector2f.DistanceSquared(p, P1);
            }
            ...
            Vector2f pB = P0 + b * v;
            closestP = pB;
            return Vector2f.DistanceSquared(p, pB);
        }
        ...
        /// </summary>
        public float DistanceSquaredPointToVirtualLine(Vector2f p)
        {
            Vector2f closestP;
        ...
        /// </summary>
        private static void FindRadialT(Vector2f p0, Vector2f p1, Vector2f p2, float dist, out float t1, out float t2)
        {
        ...
        /// <summary>
        // Rotate a Vector2f by theta radians
        /// </summary>
        private static Vector2f Rotate(Vector2f vec, float theta)
        {
            ...
            return new Vector2f(rotatedVec4.X, rotatedVec4.Y);
        }
        ...
        // return MaxFloat for the two t values.
        /// </summary>
        private void FindLinearT(Vector2f p0, Vector2f p1, float dist, out float t1, out float t2)
        {
        ...
        /// </summary>
        public void FindFirstIntersection(Vector2f p0, Vector2f p1, float dist, out float tMin)
        {
            float t1;
            float t2;
            tMin = float.MaxValue;
        ...
        /// </summary>
        public void FindNearbyLines(List<RoundLine> lineList, List<RoundLine> nearbyLineList, float globalLineRadius, Vector2f referencePos, float referenceRadius)
        {
        ...
        /// </summary>
        static public void FindNearbyLinesWithClipping(List<RoundLine> lineList, List<RoundLine> nearbyLineList, float globalLineRadius, Vector2f referencePos, float referenceRadius)
        {
            nearbyLineList.Clear();

            Vector2f C = referencePos;
            ...
            foreach (RoundLine line in lineList)
            {
                ...
                else
                {
                    ...
                    Vector2f newP0 = line.P0;
                    Vector2f newP1 = line.P1;
                    if (0 <= u2 && u2 <= 1)
                        newP0 = Vector2f.Lerp(A, B, u2);
                    if (0 <= u1 && u1 <= 1)
                        newP1 = Vector2f.Lerp(A, B, u1);
        ...
        /// </summary>
        public float MinDistanceSquaredDeviation(List<RoundLine> lineList, Vector2f currentPos, float lineRadius, float discRadius)
        {
        ...
        /// </summary>
        public void CollideAndSlide(List<RoundLine> lineList, Vector2f currentPos, Vector2f proposedPos, float lineRadius, float discRadius, out Vector2f finalPos)
        {
            Vector2f oldPos = currentPos;
            Vector2f oldTarget = proposedPos;
            bool pastFirstSlide = false;

            // Keep looping until there's no further desire to move somewhere else.
            while (Vector2f.DistanceSquared(oldPos, oldTarget) > 0.001f * 0.001f)
            {
                ...
                foreach (RoundLine line in lineList)
                {
                    ...
                    if (tMinThisLine >= 0 && tMinThisLine <= 1)
                    {
                        ...
                        Vector2f newPos = new Vector2f(MathHelper.Lerp(oldPos.X, oldTarget.X, tMinThisLine),
                        ...
                        if (line.DistanceSquaredPointToVirtualLine(newPos) - minDist2 < 0)
                        {
                            ...
                            {
                                float tc = (ta + tb) / 2;
                                Vector2f newPosC = new Vector2f(MathHelper.Lerp(oldPos.X, oldTarget.X, tc),
                    ...
                    else
                    {
                        ...
                        if (newDist - minDist2 < 0)
                        {
                            ...
                            {
                                float tc = (ta + tb) / 2;
                                Vector2f ptC = new Vector2f(MathHelper.Lerp(oldPos.X, oldTarget.X, tc),
                                                          MathHelper.Lerp(oldPos.Y, oldTarget.Y, tc));
                ...
                else
                {
                    ...
                    Vector2f newPos = new Vector2f(MathHelper.Lerp(oldPos.X, oldTarget.X, minT),
                    ...
                    if (minDistDeviation < 0)
                    {
                        ...
                        minT = ta;
                        newPos = new Vector2f(MathHelper.Lerp(oldPos.X, oldTarget.X, minT),
                                             MathHelper.Lerp(oldPos.Y, oldTarget.Y, minT));
                    }
                    ...
                    if (doSlide)
                    {
                        ...
                        Vector2f lineNormal = (newPos - closestP);
                        ...
                        Vector2f newPoint = new Vector2f(newPos.X + (float)Math.Cos(theta), newPos.Y + (float)Math.Sin(theta));
                        ...
                        Vector2f v1 = oldTarget - newPos;
                        Vector2f v2 = newPoint - newPos;
                        float dotprod = Vector2f.Dot(v1, v2);
                        ...
                        if (newTargetDist - minDist2 < 0)
                        {
                            ...
                            for (int i = 0; i < 10; i++)
                            {
                                ...
                                Vector2f newTargetTest = newTarget + (shiftAmtC * lineNormal);
