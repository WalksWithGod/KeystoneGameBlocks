...
namespace Steering
{
	public class SimpleVehicle : SteerLibrary
	{
	    ...
	    public void ApplySteeringForce(Vector3d force, double elapsedTime)
		{
			...
			if (elapsedTime > 0)
			{
                double smoothRate = Utilities.Clamp(9 * elapsedTime, 0.15f, 0.4f);
		...
		protected virtual Vector3d AdjustRawSteeringForce(Vector3d force, double deltaTime)
		{
            ...
            double cosine = Utilities.Lerp(1.0f, -1.0f, Math.Pow(range, 20));
		...
		/// <returns></returns>
        public override Vector3d PredictFuturePosition(double predictionTime)
		{
		...
		void MeasurePathCurvature(double elapsedTime)
		{
			if (elapsedTime > 0)
			{
				...
				Utilities.BlendIntoAccumulator(elapsedTime * 4.0d, Curvature, ref _smoothedCurvature);
