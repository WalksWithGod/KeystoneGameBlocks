...
namespace Steering.Database
{
	...
	public class LocalityQueryProximityDatabase<T> : IProximityDatabase<T> where T : class
	{
		...
		private sealed class TokenType : ITokenForProximityDatabase<T>
		{
            ...
            public void UpdateForNewPosition(Vector3d p)
			{
				_lq.UpdateForNewLocation(_proxy, p);
			}

			// find all neighbors within the given sphere (as center and radius)
            public void FindNeighbors(Vector3d center, double radius, List<T> results)
			{
			...
			// clientQueryState
		    private static void perNeighborCallBackFunction(Object clientObject, double distanceSquared, Object clientQueryState)
			{
		...
		// constructor
        public LocalityQueryProximityDatabase(Vector3d center, Vector3d dimensions, Vector3d divisions)
		{
			Vector3d halfsize = dimensions * 0.5f;
			Vector3d origin = center - halfsize;
