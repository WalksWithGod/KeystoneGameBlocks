...
namespace Keystone.Celestial
{
    public class GenerateStar
    {
        ...
        // pluto is 39.5 AU.  Our regions are 500 AU diameter (250 AU across).
        // No planet's orbit should extend beyond the region and probably even more like 150 AU 
        // So we can safely shift stellar systems by a random amount +- up to 75 AU in any direction to make it seem
        // less gridlike
        private const float MAX_STAR_DIAMETER_AU = 39.5f; 

        private float _percentTypeM,
                      _percentTypeK,
                      _percentTypeG,
                      _percentTypeF,
                      _percentTypeA,
        ...
        private float _percentSupergiantI,
                      _percentGiantII,
                      _percentGiantIII,
                      _percentSubgiantIV,
                      _percentDwarfV,
                      ...
                      _percentWhiteDwarfD,
                      _percentPulsarP,
                      _percentNeutronStarN,
                      _percentBlackHoleB;


        //Constants for the Default (Realistic) Luminosity Class Ratios. These must equal 100% when added together
        private const float RATIO_SUPERGIANT_I = 0.03f;
        private const float RATIO_GIANT_II = 0.07f;
        private const float RATIO_GIANT_III = .0325f;
        private const float RATIO_SUBGIANT_IV = .03f;
        private const float RATIO_DWARF_V = .6875f; // mainsequence
        private const float RATIO_SUBDWARF_VI = .042f;
        private const float RATIO_WHITEDWARF_D = .024f;
        private const float RATIO_PULSAR_P = .01f;
        private const float RATIO_NEUTRONSTAR_N = .01f;
        private const float RATIO_BLACKHOLE_B = .01f;

        //constants for the Default (realistic) Spectral Type ratios
        private const float RATIO_TYPE_O = 0.4f;
        private const float RATIO_TYPE_B = 0.6f;
        private const float RATIO_TYPE_A = 11;
        private const float RATIO_TYPE_F = 11;
        private const float RATIO_TYPE_G = 12;
        private const float RATIO_TYPE_K = 15;
        private const float RATIO_TYPE_M = 50;


        private float[][,] _luminosities = new float[6][,];
        private float[][,] _masses = new float[6][,];

        private int[,] _temperatures = new int[,]
                                           {
                                               {58000, 56000, 54000, 51000, 48000, 45000, 42000, 39000, 36000, 33000},
        ...
        public GenerateStar(Random random)
        {
        ...
        public void ComputePrimaryStatistics(Star newstar)
        {
            ...
            newstar.Temperature = GetStarTemperature(newstar.LuminosityClass, newstar.SpectralType,
                                                     newstar.SpectralSubType);
            newstar.Luminosity = GetStarLuminosity(newstar.LuminosityClass, newstar.SpectralType,
                                                   newstar.SpectralSubType);
            newstar.MassKg = GetStarMass(newstar.LuminosityClass, newstar.SpectralType, newstar.SpectralSubType);
            newstar.Diameter = GetStarDiameterAU(newstar.Luminosity, newstar.Temperature);
            newstar.StableLifespan = GetStarStableLifespan(newstar.MassKg, newstar.Luminosity);
        ...
        public void ComputeCompanionStatistics(Star companion, Star primary)
        {
            ...
            companion.Age = primary.Age;
            companion.StableLifespan = Math.Max(primary.Age, GetStarStableLifespan(companion.MassKg, companion.Luminosity));
            // companion star ages are same as primary since they formed together.  
            // just make sure that the stableLifeSpan is > the star's age 
        ...
        public LUMINOSITY GetLuminosityClass()
        {
            ...
            float i = (float)_random.NextDouble();
            ...
            if (i <= _percentSupergiantI)
                ...
                result = LUMINOSITY.SUBGIANT_IV;
                     ...
                     (_percentSupergiantI + _percentGiantII + _percentGiantIII + _percentSubgiantIV + _percentDwarfV))
                      ...
                      + _percentSubdwarfVI + _percentWhiteDwarfD))
            {
                result = LUMINOSITY.WHITEDWARF_D;
                     ...
                     (_percentSupergiantI + _percentGiantII + _percentGiantIII + _percentSubgiantIV +
                      _percentDwarfV
        ...
        private LUMINOSITY GetCompanionLuminosityClass(LUMINOSITY PrimaryLuminosityClass)
        {
        ...
        private SPECTRAL_TYPE GetSpectralType(LUMINOSITY LuminosityClass)
        {
            ...
            double i = _random.NextDouble() * 100 ; //  Generate random single between 1 and 100.
            if (i <= _percentTypeM)
        ...
        private SPECTRAL_TYPE GetCompanionSpectralType(LUMINOSITY LuminosityClass, LUMINOSITY PrimaryLuminosityClass,
                                                       SPECTRAL_TYPE PrimarySpectralType)
        {
        ...
        private SPECTRAL_SUB_TYPE GetSpectralSubType(SPECTRAL_TYPE SpectralType)
        {
            ...
            if (SpectralType == SPECTRAL_TYPE.O)
                ...
                i = _random.Next(5, 10); // generate value between 5 and 9
            else
                i = _random.Next(0, 10); //  Generate random value between 0 and 9.

            return (SPECTRAL_SUB_TYPE) i;
        }

        private SPECTRAL_SUB_TYPE GetCompanionSpectralSubType(LUMINOSITY LuminosityClass,
                                                              LUMINOSITY PrimaryLuminosityClass,
        ...
        // distance in AU 
        private double GetStarOrbitalEccentricity(double distance)
        {
            return 1.0d; // for now simply use perfect circle.  
        ...
        /// <summary>
        /// Calculates the effective temperature in Kelvin
        /// </summary>
        /// <param name="LuminosityClass"></param>
        /// <param name="SpectralType"></param>
        /// <param name="SubType"></param>
        /// <returns></returns>
        private int GetStarTemperature(LUMINOSITY LuminosityClass, SPECTRAL_TYPE SpectralType, SPECTRAL_SUB_TYPE SubType)
        {
            int result;
        ...
        // TODO: i originally cut the random.Next values used here for the luminosity types
        // that do not pull values from a table from the GetTempterature() function and have subseqently
        // tried to go back and fix some of these values, but it's not perfect
        private float GetStarLuminosity(LUMINOSITY LuminosityClass, SPECTRAL_TYPE SpectralType,
                                        SPECTRAL_SUB_TYPE SubType)
        {
            float result;
            switch (LuminosityClass)
            {
                case LUMINOSITY.WHITEDWARF_D:
                    result = _random.Next(400, 850);
                    break;
                case LUMINOSITY.NEUTRON_N:
                    result = _random.Next(700, 1200);
                    break;
                case LUMINOSITY.PULSAR_P:
                    result = _random.Next(1000, 3500);
                    break;
                case LUMINOSITY.BLACKHOLE_B:
                    result = 0.0001f;
                    break;
            ...
            result *= (float) _random.Next(85, 105)/100; // must include the (float) cast or you get int values of 0
            return result;
        }

        private float GetStarDiameterAU(float luminosity, int temperature)
        {
            ...
            float diameterAU = (float)(309000*( Math.Sqrt(luminosity) / (temperature * temperature)));
            float result = (float)(Temp.AU_TO_METERS * diameterAU );
            float maxDiameterMeters = MAX_STAR_DIAMETER_AU * (float)Temp.AU_TO_METERS;
                        
            return Math.Min (maxDiameterMeters , result);
        }

        /// <summary>
        /// Returns star mass in kilograms
        /// </summary>
        /// <param name="luminosityClass"></param>
        /// <param name="spectralType"></param>
        /// <param name="subType"></param>
        /// <returns></returns>
        private double GetStarMass(LUMINOSITY luminosityClass, SPECTRAL_TYPE spectralType, SPECTRAL_SUB_TYPE subType)
        {
            ...
            {
                case LUMINOSITY.WHITEDWARF_D:
                    result = _random.Next(1, 2); // 1.4 solar masses technically seems to be limit beyond which a supernova would occur rather than degenerating into a white dwarf
                    break;
                case LUMINOSITY.NEUTRON_N:
                    result = _random.Next(1, 4);
                    break;
                case LUMINOSITY.PULSAR_P:
                    result = (float)_random.NextDouble() + .005f; // pulsars are extemely dense, but are not very massive
                    break;
                case LUMINOSITY.BLACKHOLE_B:
                    result = 0.0001f;
                    break;
            ...
            return result * Temp.SOL_MASS_KILOGRAMS;
        }

        /// <summary>
        /// Returns star stable lifespan in billions of years
        /// </summary>
        /// <param name="mass"></param>
        /// <param name="luminosity"></param>
        /// <returns></returns>
        private float GetStarStableLifespan(double mass, float luminosity)
        {
            // This function accepts the mass and luminosity of a star
            // and from it calculates the Stable Lifespan of that star
            return (float)(10*mass / Temp.SOL_MASS_KILOGRAMS)/luminosity;
        }

        /// <summary>
        /// Star age in billions of years.  This should be in seconds so it's more generic?
        /// </summary>
        /// <param name="stableLifespan"></param>
        /// <returns></returns>
        private float GetStarAge(float stableLifespan)
        {
        ...
        private void InitMainSequenceLuminosities()
        {
            _luminosities[0] = new float[,]
                                   {
                                       {
                                           1647400, 1537920, 1428440, 1318960, 1209480, 1100000, 990520, 881040, 771560,
                                           662080
                                       },
            ...
            _luminosities[4] = new float[,]
                                   {
                                       {
                                           1184585, 1105668, 1026751, 947834, 868917, 790000, 711083, 632166, 553249,
                                           474332
                                       }
                                       ...
                                       ,
                                       {
                                           0.063f, 0.0526f, 0.0422f, 0.0318f, 0.0214f, 0.011f, 0.0077f, 0.0043f, 0.001f,
                                           0.0005f
                                       }
                                   };
            ...
            _luminosities[5] = new float[,]
                                   {
                                       ...
                                       {
                                           0.15f, 0.1358f, 0.1216f, 0.1074f, 0.0932f, 0.079f, 0.0716f, 0.0642f, 0.0568f,
                                           0.0494f
                                       },
                                       {
                                           0.042f, 0.0366f, 0.0312f, 0.0258f, 0.0204f, 0.015f, 0.01326f, 0.01152f, 0.00978f
                                           , 0.00804f
                                       },
                                       {
                                           0.0063f, 0.00526f, 0.00422f, 0.00318f, 0.00214f, 0.0011f, 0.00077f, 0.00043f,
                                           0.0001f, 0.00005f
                                       }
                                   };
        }
            ...
            _masses[5] = new float[,]
                             {
                                 ...
                                 ,
                                 {
                                     0.042f, 0.0366f, 0.0312f, 0.0258f, 0.0204f, 0.015f, 0.01326f, 0.0152f, 0.0978f,
                                     0.0804f
                                 }
                                 ,
