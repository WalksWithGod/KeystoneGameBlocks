...
namespace Keystone.IO
{
    ...
    public class NodeState : Node
    {
        ...
        {
            // todo: this class seems not fully realized.  i never properly tested it.
            //       maybe i should just set entity.Ref = null after loading the entity so that we dont refer to a prefab and so that our serialization avoids using NodeState.cs until some day when we fully realize it.
            // todo: the only thing we need to do really is make sure the server side node.IDs match the client side IDs
            Shareable = false;
        }
