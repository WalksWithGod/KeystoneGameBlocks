...
namespace Paloma
{
...

    {
        ...
        public TargaImage(System.IO.Stream steam)
            : this()
        {
            if (steam != null && steam.Length > 0 && steam.CanSeek == true)
            {
                BinaryReader binReader = null;
                // create a BinaryReader used to read the Targa file
                using (binReader = new BinaryReader(steam))
                {
                ...
                throw new Exception(@"Error loading Targa, could not read Targa from stream.");
        }
            ...
            switch (this.objTargaHeader.PixelDepth)
            {
                ...
                case 32:
                    ...
                    if (this.Format == TGAFormat.NEW_TGA)
                    {
                        switch (this.objTargaExtensionArea.AttributesType)
                        {
                            case 0: // Hypnotron Oct.19.2011 - included case 0 here instead per http://www.codeproject.com/Messages/4037014/One-small-correction.aspx
                            case 1:
                            case 2: // no alpha data
                                pfTargaPixelFormat = PixelFormat.Format32bppRgb;
                                break;

                            // case 0: // Hypnotron Oct.19.2011 - commented out case 0 here per suggestion (see case 0 above)
                            case 3: // useful alpha data
        ...
        public static Bitmap LoadTargaImage(Stream stream)
        {
            Bitmap b = null;
            using (TargaImage ti = new TargaImage(stream))
            {
                b = new Bitmap(ti.Image);
            }
