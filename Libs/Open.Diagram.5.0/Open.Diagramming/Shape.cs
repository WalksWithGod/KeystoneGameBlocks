...
namespace Open.Diagramming
{
	public class Shape:Solid, ICloneable, ISelectable, IUserInteractive, IPortContainer
	{
		...
		public Shape(Shape prototype): base(prototype)
		{
			...
			_allowScale = prototype.AllowScale;
			...
			_maximumSize = prototype.MaximumSize;
			_minimumSize = prototype.MinimumSize;
		...
		protected SizeF ValidateSize(float width, float height)
		{
			...
			{
				if (width < _minimumSize.Width) width = _minimumSize.Width;
				if (width > _maximumSize.Width) width = _maximumSize.Width;
			}
			
			if (!KeepAspect || height > width)
			{
				if (height < _minimumSize.Height) height = _minimumSize.Height;
				if (height > _maximumSize.Height) height = _maximumSize.Height;
			}
		...
		private void ScaleShape(float sx, float sy, float dx, float dy, bool maintainAspect)
		{
			...
			if (!KeepAspect || Width >= Height)
			{
				if (_maximumSize.Width < (Bounds.Width * sx)) sx = Convert.ToSingle(_maximumSize.Width / Bounds.Width);
				if (_minimumSize.Width > (Bounds.Width * sx)) sx = Convert.ToSingle(_minimumSize.Width / Bounds.Width);
			}
			
			if (!KeepAspect || Height >= Width)
			{
				if (_maximumSize.Height < (Bounds.Height * sy)) sy = Convert.ToSingle(_maximumSize.Height / Bounds.Height);
				if (_minimumSize.Height > (Bounds.Height * sy)) sy = Convert.ToSingle(_minimumSize.Height / Bounds.Height);
			}
