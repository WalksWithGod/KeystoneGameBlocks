...
namespace Open.Diagramming.Forms.Rendering
{
	public class Render
	{
        ...
        private Type _lastRenderType = null;
        private IFormsRenderer _lastRenderer = null;
        ...
        public virtual IFormsRenderer GetRenderer(Type type)
        {
            if (type == null) throw new ArgumentNullException("type");
            if (type == _lastRenderType) return _lastRenderer;

            //Lookup renderer for type
            IFormsRenderer render = null;
            _renderers.TryGetValue(type, out render);
            if (render == null) throw new RenderException(string.Format("A renderer was not found for type {0}.", type.FullName));

            //Cache renderer for next lookup
            _lastRenderer = render;
            return render;
        }
		...
		public virtual void RenderDiagram(Rectangle renderRectangle)
		{
            ...
            if (! Suspended)
			{
				if (! renderRectangle.Equals(_renderRectangle)) DisposeGraphicsStateBitmap();
				
				Graphics graphics = GetGraphics(renderRectangle);
				if (graphics==null) return;

                RenderEventArgs e = new RenderEventArgs(graphics, renderRectangle);
				OnPreRender(e);

                RenderLayers(graphics, renderRectangle);
		...
		public virtual Graphics GetGraphics(Rectangle renderRectangle)
		{
			...
			try
			{
				if (_graphicsStateBitmap == null)
				{
					...
					if (Paging.Enabled) 
					{
						...
						RenderPage(graphics,Paging.WorkspaceOffset,Paging.PageSize);
		...
		public virtual void RenderLayers(Graphics graphics, Rectangle renderRectangle)
		{
			...
			if (layer.DrawShadows)
			{
				foreach (Element element in elementRenderList)
				{
                    if (element.Layer == layer && element.DrawShadow && element.Visible && element.Bounds.IntersectsWith(renderRectangle) && (element.Group == null || element.Group.Expanded))
					{
                        ...
                        renderer = GetRenderer(element.GetType());
		...
		protected virtual void RenderPage(Graphics graphics, Point workspaceOffset, SizeF pageSize)
		{
			graphics.Clear(Paging.WorkspaceColor);

			//Draw border
			Pen pen = new Pen(Color.FromArgb(66,65,66));
			RectangleF border = new RectangleF(workspaceOffset, pageSize);
			border.Inflate(1,1);
