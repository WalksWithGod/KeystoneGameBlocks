...
{
	[ToolboxBitmap(typeof(Open.Diagramming.Forms.Palette), "Resource.palette.bmp")]
	public class Palette : Diagram
	{
		...
		protected override void OnDragEnter(DragEventArgs drgevent)
		{
			...
			if (CurrentMouseElements.MouseStartElement != null) 
			{
				...
				dragPoint = OffsetDrop(dragPoint,CurrentMouseElements.MouseStartElement.Bounds);
				
				drgevent.Effect = DragDropEffects.Move;

				Render.Lock();
				Render.DecorationPath = CurrentMouseElements.MouseStartElement.GetPath();

				//Move action path from origin to the mouse position
				Matrix matrix = new Matrix();
				matrix.Translate(dragPoint.X,dragPoint.Y);
				Render.DecorationPath.Transform(matrix);
				matrix.Dispose();
			}
		...
		private void ArrangeImplementation(float spacingWidth, float spacingHeight)
		{
			...
			_lastCols = (int) System.Math.Floor((DisplayRectangle.Width - Margin.Left - Margin.Right) / Spacing.Width);
