...
namespace HtmlRenderer.Dom
{
    ...
    internal class CssBox : CssBoxProperties, IDisposable
    {
        ...
        /// <summary>
        /// If the box "Display" is "Inline", then this is an inline box and not block.
        /// </summary>
        ...
        /// <summary>
        /// If the box HTML tag is a flow tag, then this is a a flow element.
        /// </summary>
        public bool IsFlowElement
        {
        	get { return (_htmltag == null ? false : 
        	              _htmltag.Name.Equals("div", StringComparison.InvariantCultureIgnoreCase) || 
        	              _htmltag.Name.Equals("dt", StringComparison.InvariantCultureIgnoreCase) || 
        	              _htmltag.Name.Equals("dd", StringComparison.InvariantCultureIgnoreCase) || 
        	              _htmltag.Name.Equals("dl", StringComparison.InvariantCultureIgnoreCase) || 
        	              _htmltag.Name.Equals("ul", StringComparison.InvariantCultureIgnoreCase) || 
						  _htmltag.Name.Equals("ol", StringComparison.InvariantCultureIgnoreCase) || 
						  _htmltag.Name.Equals("li", StringComparison.InvariantCultureIgnoreCase)); }
        }
        
        /// <summary>
        /// Is the box "Display" is "Block", then this is a block box and not inline.
        /// </summary>
        public bool IsBlock
        {
        ...
        public virtual bool IsClickable
        {
            get 
            {
            	...
            	// test if it has an "active" psuedoclass 
            	bool hasFocus = HtmlTag.HasAttribute ("active") || HtmlTag.HasAttribute(":active");
            	if (hasFocus )
            		System.Diagnostics.Debug.WriteLine ("has focus");
            	
        		return HtmlTag.Name == HtmlConstants.A && !HtmlTag.HasAttribute("id");
        	}
        }
        ...
        public static CssBox CreateBox(HtmlTag tag, CssBox parent = null)
        {
            ...
            {
            	// TODO: i believe this fails when the background-image is specified in a style
            	//       instead of as a "img" tag.  The CssBox must be changed to a CssBoxImage if the style
        ...
        public void Paint(IGraphics g)
        {
            try
            {
                if (Display != CssConstants.None && Visibility == CssConstants.Visible)
                {
                    ...
                    // TODO: Hypnotron Oct.22.2013 - Can we skip rendering also if no change has occurred?
                    // i mean, obviously if we need to refresh, we need to refresh, but if we just want to update
                    // a particular part of the area... i think it means we need to do our own double buffering
        ...
        protected virtual void PerformLayoutImp(IGraphics g)
        {
            ...
            if( IsBlock || Display == CssConstants.ListItem || Display == CssConstants.Table || Display == CssConstants.InlineTable || Display == CssConstants.TableCell )
            {
                ...
                if( Display != CssConstants.TableCell )
                {
                    ...
                    // TODO: Hypnotron Dec.26.2013 - top calculation for our nested div column is not computed correctly.  still trying to understand
                    //       the how the css spec should be applied and this code to determine solution.  
                    //       So far it seems to be finding the previous sibling as a div containing an icon and using it's 'bottom" 
                    //       and perhaps div siblings of div boxes should be ignored as valid siblings?
                    //       or actually, if this is a div and previous sibling is a div, then if the previous sibling's left and right positions are both < this boxes left
                    //       we can use the same top as the containing block
                    //top = ContainingBlock.Location.Y + ContainingBlock.ActualPaddingTop + ActualMarginTop + ContainingBlock.ActualBorderTopWidth;
                    // div is a type of "flow element" http://www.w3.org/TR/html-markup/common-models.html#common.elem.flow
                    if (prevSibling != null)
                    {
                    	if (this.HtmlTag != null && this.IsFlowElement && prevSibling.HtmlTag != null && prevSibling.IsFlowElement)
                    	{
                    		//if (prevSibling.Location.X + prevSibling.Size.Width < left)
                    		if (prevSibling.Location.X  < left)
	                    		top = ContainingBlock.Location.Y + ContainingBlock.ActualPaddingTop + ActualMarginTop + ContainingBlock.ActualBorderTopWidth;
                    	}
                    }
                    Location = new PointF(left, top);
                    ActualBottom = top; // minimum value of 'ActualBottom' initialized to equal newly computed top
                }
        ...
        {
        	// TODO: this does not seems to inherit in a way that supports nesting where
        	//       in particular border, margin, padding, alignment, information is inherited and combined with
        	//       the child's relative border, margin, padding, alignment information
        ...
        protected virtual void PaintImp(IGraphics g)
        {
            ...
            if (Display != CssConstants.None && (Display != CssConstants.TableCell || EmptyCells != CssConstants.Hide || !IsSpaceOrEmpty))
            {
                ...
                // TODO: below we need for child boxes to inherit any attributes of parents so that they can be drawn independantly
                // if just they are dirty and not entire parent, yet still have (as an examle) the proper background of the parent
        	...
        	catch (Exception ex )
        	{
        		...
        		throw ex; // catch it so we know what it is.
        	}
        }
        ...
        protected void PaintBackground(IGraphics g, RectangleF rect, bool isFirst, bool isLast)
        {
            if (rect.Width > 0 && rect.Height > 0)
            {
                ...
                {
                	// TODO: all GDI elements should be cached! well, some sights say that this is not true.  they should be disposed immediately after use
                ...
                // Nov.11.2013 - Hypnotron - Background Image drawing moved AFTER background color painting
        ...
        // TODO: ideally here, each "word" or "rect" area is also an item that can be drawn and updated seperately
        //       for instance, each CssBox fragment can contain within it a list of "cells" and i believe are guaranteed to never overlap
        // http://tech.pro/tutorial/773/winforms-tutorial-manage-your-own-double-buffering
        //      or perhaps rather than just a list of cells, we should create a graph of cells so that z-order can also be tracked since
        //      apparently it IS possible to have css boxes overlaps.  indeed, a table is a cssBox that then contains more cssBoxes right?
        //      and a graph would then inherit attributes that were above it.
        //      - well i think now we're back to the idea that our CssBox classes already are the graph we're talking about.  
        // TODO: since html changes have to always be completely reparsed, can we do a DIFF of the two resulting graphs (before vs after)
        //       and find which boxes have changed in any way and then find the areas that need to be updated and find the graphics attributes
        //       needed for eahc and just update those?
        /// <summary>
        ...
        {
            _rectangles.Clear(); // TODO: nowhere are rectangles ever being added here.
        }
