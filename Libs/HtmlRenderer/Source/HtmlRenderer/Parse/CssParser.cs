...
namespace HtmlRenderer.Parse
{
    ...
    internal static class CssParser
    {
        ...
        /// <param name="blockSource">the CSS block to parse</param>
        /// <returns>the created CSS block instance</returns>
        private static CssBlock ParseCssBlockImp(string classInfo, string blockSource)
        {
        	
        	
        	string psedoClass = null;
            var colonIdx = classInfo.IndexOf(":", StringComparison.Ordinal);
            if (colonIdx > -1 && !classInfo.StartsWith("::"))
            {
                psedoClass = colonIdx < classInfo.Length - 1 ? classInfo.Substring(colonIdx + 1).Trim() : null;
                classInfo = classInfo.Substring(0, colonIdx).Trim();
            }

            // TODO: why would it stop if pseudoClass was something other than "link" or "hover" ?
            if (!string.IsNullOrEmpty(classInfo) && (psedoClass == null || psedoClass == "link" || psedoClass == "hover"))
            {
                string firstClass;
                var selectors = ParseCssBlockSelector(classInfo, out firstClass);
...
//          TODO: add better psuedo-class parsing support
//        	string className;
//        	string[] psuedoClass = ParseCSSPseudoClasses(classText, out className);
//
//
//        	if (!string.IsNullOrEmpty(className) &&))
//            {
...
//
//                
//                return new CssBlock(firstClass, properties, selectors, psuedoClass.Contains("hover"));
//            }
//
//            return null;
        }
        ...
        private static string[] ParseCSSPseudoClasses (string classText, out string className)
        {
        	// A pseudo-class starts with a colon (:). No whitespace may appear between
			// a type selector or universal selector and the colon, nor can whitespace appear after the colon.
        		
        	var colonIdx = classText.IndexOf(":", StringComparison.Ordinal);
            if (colonIdx > -1 && !classText.StartsWith("::"))
            {
            	if (classText.Contains ("focus"))
            		System.Diagnostics.Debug.WriteLine ("has focus");
            	
                string psuedoClass = colonIdx < classText.Length - 1 ? classText.Substring(colonIdx + 1).Trim() : null;
                className = classText.Substring(0, colonIdx).Trim();
                return new string[]{psuedoClass};
            }
        ...
        private static void AddProperty(string propName, string propValue, Dictionary<string, string> properties)
        {
            ...
            // TODO: "display" is a property with values "none", "block", "inline", ???, "focus" <-- focus is an event
            // 
            if (propName == "width" || propName == "height" || propName == "lineheight")
            {
                ParseLengthProperty(propName, propValue, properties);
            }
            else if (propName == "color" || propName == "background-color" || propName == "backgroundcolor" || propName == "bordertopcolor" || propName == "borderbottomcolor" || propName == "borderleftcolor" || propName == "borderrightcolor")
            {
            ...
            {
            	//System.Diagnostics.Debug.WriteLine ("CssParser.AddProperty() - Unable to parse '" + propName + "'.  Adding 'as is'.");
        ...
        /// <summary>
        /// Parse the complex shorthand Background property value that contains multiple css properties into specific css properties.
        /// </summary>
        private static void ParseBackgroundProperty (string propValue, Dictionary<string, string> properties)
        {
    	    // "background" shorthand property can potentially have array of values for color, image, repeat, attachment, and position
        	// note that the order is mandatory in the Css specification, although any particular value can be omitted.  However, omitted
        	// values are treated as default values for those properties (eg. null) and so will reset those properties.  Thus the "background"
        	// shorthand property should be thought of as _always_ pushing all values (color, image, repeat, attachment, and position) on the Css
        	// hierarchical stack, even for omitted values.
        	// note: we add each value as the full value rather than the shorthand
        	string url = ParseBackgroundImageProperty (propValue );
            properties["background-image"] = url;
            
            // split the string by  " "
            string color = "#CCCCCC"; // null; // TODO: remove hardcode and parse actual color 
            string repeat = RegexParserUtils.Search(RegexParserUtils.CssBackgroundRepeat, propValue);
            string attachment = RegexParserUtils.Search(RegexParserUtils.CssBackgroundAttachment, propValue);
            string position = RegexParserUtils.Search(RegexParserUtils.CssBackgroundPosition, propValue); 
            
            // TODO: temp hack implementation
            properties["background-color"] = color;
            properties["background-repeat"] = repeat;
            properties["background-position"] = "center right"; // position;
