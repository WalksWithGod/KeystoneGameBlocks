...
namespace HtmlRenderer
{
    ...
    public sealed class HtmlContainer : IDisposable
    {
        ...
        /// <summary>
        /// Raised when mouse enters over a link in the html.<br/>
        /// </summary>
        /// <remarks>Hypnotron Oct.25.2013 - Event Added</remarks>
        public event EventHandler<HtmlLinkClickedEventArgs> LinkMouseEnter;
        
        /// <summary>
        /// Raised when mouse leaves the previous mouse over link in the html.<br/>
        /// </summary>
        /// <remarks>Hypnotron Oct.25.2013 - Event Added</remarks>
        public event EventHandler<HtmlLinkClickedEventArgs> LinkMouseLeave;
        ...
        public void SetHtml(string htmlSource, CssData baseCssData = null)
        {
            ...
            // TODO: CssBox classes are all internal classes.  So, if we wanted to manage a DIFF
            // we should do it here by caching previousRoot with current Root
            if (!string.IsNullOrEmpty(htmlSource))
            {
                _cssData = baseCssData ?? CssUtils.DefaultCssData;


				Dom.CssBox root = HtmlParser.ParseDocument(htmlSource);
				
				
        		CssBox newRoot = DomParser.GenerateCssTree(root, this, ref _cssData);
				           
                // TODO: .GenerateCssTree should be split into a CSS DOM Graph which can then be traversed
                //        on a seperate pass to create a renderable tree of visuals.  Having the tree is good for
                //        being able to do a diff and a partial update.
				//        Instead below it creates a render graph that has no ability
                //        to branch on the fly as needed based on browser settings.
                
                // note: this tree is not a proper DOM.  It's a final renderable. 
				// The problem with this is, with a proper DOM, all paths are loaded and render traversal can change
				// based on state of the browser and some other state vars.  But as it is, a final render graph is
				// created and the only way to change it is to call GenerateCssTree again.        		
               // CssBox newRoot = DomParser.GenerateCssTree(htmlSource, this, ref _cssData);
                if (_root != null)
                {
                	// perform diff and if the structure has not changed, then it's just values that have changed
                	// and we can get the before and after rects. If both before and after rect for a specific box 
                	// has not changed, we need do nothing.  If they after has gotten bigger we do nothing.  If the after is now smaller
                	// then we must recurse upwards until we have parent/grandparent boxes that encompass the former size
                	// Then when we performPaint, we only want to paint those list of boxes
                }
                _root = newRoot;
                if (_root != null)
                {
        ...
        public void PerformPaint(Graphics g)
        {
            ...
            // TODO: if there is a list of DIFF nodes, then redraw those on the backbuffer intsead?
			//        we can still use the previous layout
			//       this PerformPaint() should only occur when changes have occurred 
			//       either to the Tree or the layout of the control (eg resized).  For cases
			//       where focus has regained, we just re-blit the existing double buffer.
            if (_root != null)
            {
            	// TODO: this is problematic, we are creating a new WinGraphics every frame and the problem is that
            	//       we are not cleaning up the old one properly i dont think
                using (var ig = new WinGraphics(g))
                {
                    _root.Paint(ig);
                }
        ...
        private CssBox _previousLinkMouseEnter;
        ...
        public void HandleMouseMove(Control parent, MouseEventArgs e)
        {
            ...
            try
            {
                ...
                // TODO: hover needs to respond not just to links, but any box that has :hover pseudo-class assigned to it.
                // Handle mouse hover over the html to change the cursor depending if hovering word, link of other.
                var link = DomUtils.GetLinkBox(_root, loc);              
                
                // Hypnotron Oct.28.2013 - Added LinkMouseLeave and LinkMouseEnter event determination and firing
                if (_previousLinkMouseEnter != null)
                {
                	HtmlLinkClickedEventArgs previousArgs = new HtmlLinkClickedEventArgs(_previousLinkMouseEnter.HrefLink, _previousLinkMouseEnter.HtmlTag.Attributes);
                	
                	if (link == null)
                	{
                		// TODO: why is this returning null for link when over the link rect? is it
                		System.Diagnostics.Debug.WriteLine ("Link == null " + loc.ToString()); // 
                		if (LinkMouseLeave != null)
	            			LinkMouseLeave(parent, previousArgs);
                	}
                	else if (link.Text != _previousLinkMouseEnter.Text)
                	{
                		// TODO: i have a major problem where when i set the HTML after making a tiny modification
                		//       the CssBox references all change!  So there is no way I can do a comparison between previous and current
                		//       
                   		System.Diagnostics.Debug.WriteLine ("Link != Previous " + loc.ToString()); // 
                		 if (LinkMouseLeave != null)
	            			 LinkMouseLeave(parent, previousArgs);
	               		 
    					 if (LinkMouseEnter != null)
    					 {
		           		 	var currentArgs = new HtmlLinkClickedEventArgs(link.HrefLink, link.HtmlTag.Attributes);
    					 	LinkMouseEnter(parent, currentArgs);
    					 }
                	}
                	else // link is same as before, do nothing
                	{
                	}
                }
                else if (link != null)
                	if (LinkMouseEnter != null)
                	{
                		System.Diagnostics.Debug.WriteLine ("Link NOT null " + loc.ToString()); // 
		           	 	 var currentArgs = new HtmlLinkClickedEventArgs(link.HrefLink, link.HtmlTag.Attributes);
                		LinkMouseEnter(parent, currentArgs);
                	}
                
           		_previousLinkMouseEnter = link;
...
//                }
        ...
        internal void HandleLinkClicked(Control parent, MouseEventArgs e, CssBox link)
        {
            ...
            string hrefLink = link.HrefLink ;
        	            
        	if (LinkClicked != null)
            {
                var args = new HtmlLinkClickedEventArgs(hrefLink, link.HtmlTag.Attributes);
            ...
            if (!string.IsNullOrEmpty(hrefLink))
            {
                if (hrefLink.StartsWith("#"))
                {
                    ...
                    {
                        var box = DomUtils.GetBoxById(_root, hrefLink.Substring(1));
                        if (box != null)
                        {
                ...
                {
                	try // Hypnotron Oct.24.2013 - added try/catch
                	{
                    	var nfo = new ProcessStartInfo(hrefLink);
