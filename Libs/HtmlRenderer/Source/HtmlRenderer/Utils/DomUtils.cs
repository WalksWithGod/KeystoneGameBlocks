    ...
    internal sealed class DomUtils
    {
        ...
        public static CssBox GetPreviousSibling(CssBox b)
        {
            if (b.ParentBox != null)
            {
                ...
                {
                    int diff = 1;
                    // find obvious sibling easily
                    CssBox sib = b.ParentBox.Boxes[index - diff];

                    // however, we will look for another sibling if the obvious one does not qualify by being undisplayed or using .Absolute position
