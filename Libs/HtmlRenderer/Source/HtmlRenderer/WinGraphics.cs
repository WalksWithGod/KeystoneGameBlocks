...
namespace HtmlRenderer
{
    ...
    internal sealed class WinGraphics : IGraphics
    {
        ...
        /// <summary>
        /// Track original Font so we can restore it on cleanup
        /// </summary>
        private IntPtr _hOriginalFont;
        #endregion
        ...
        public void Dispose()
        {
        	...
        	if (_hdc != IntPtr.Zero)
	        	Win32Utils.SelectObject(_hdc, _hOriginalFont);
        ...
        /// <summary>
        /// Set a resource (eg. a font) for the specified device context.
        /// WARNING: Calling Font.ToHfont() many times without releasing the font handle crashes the app.
        /// </summary>
        private void SetFont(Font font)
        {
            InitHdc(); 
            // TODO: if this object is already selected, will mutliple calls do nothing?
            // also, what about the original font?  we are not caching it so we can restore the gdi state
            int previousFont = Win32Utils.SelectObject(_hdc, FontsUtils.GetCachedHFont(font));
            if (_hOriginalFont == IntPtr.Zero)
            	_hOriginalFont = new IntPtr (previousFont);
        }
