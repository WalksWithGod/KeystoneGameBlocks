    ...
    class Server : MarshalByRefObject {
        ...
        public string RootUrl {
            get {
                if (_port != 80) 
                {
                    return _address + ":" + _port + _virtualPath;
                }
                
                return _address + ":" + _virtualPath;
            }
        }
        ...
        public void Start(string bindToAddress) 
        {
            IPAddress address;
            if (!IPAddress.TryParse(bindToAddress, out address)) return;
            try 
            {
