	...
	internal class STPPerformanceCounters
	{
		...
		private void SetupCategory()
		{
			if (!PerformanceCounterCategory.Exists(_stpCategoryName))
			{
			    ...
			    // MPJ - NOTE: This will fail if the security permissions are not set properly.
			    // For instance, a user account (as opposed to Admin) will not have
			    // permissions to modify the registry.
			    // Typically this shouldn't be a problem.  For retail releases where
			    // an active performance counter wont be used, simply don't use them.
