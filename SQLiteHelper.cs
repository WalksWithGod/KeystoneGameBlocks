...
//When the people running the SourceForge.Net project for the SQLite.NET ADO.NET 
//provider came out with a revision to match SQLite 3.XX, I decided it would be 
//fun (and useful) to see if I could write a "SQLIteHelper.cs" class that mirrors 
//the functionality of the Microsoft.ApplicationBlocks.Data "SqlHelper" class. 
//Turns out, its really very easy. Of course, the only thing missing is stored 
//procedure support, but if you like SQLite, this helper class is for you. 
//I even implemented the object[] parameters overloads where you can pass an 
//object array of parameter values into the various static methods and the class 
//will create a SQLiteParameter collection of typed parameters and populate it for 
//you. First, here's the code. I haven't implemented everything yet, but it is
//pretty much feature complete: 

//todo: MPJ - this is helpful but I dont like having to close the connection every time... unless its not a performance hit whatsoever?
//
using System;
using System.Data;
...
namespace Finisar.SQLite
{
    ...
    public class SQLiteHelper
    {
        ...
        public static SQLiteParameter CreateParameter(string parameterName,
                                                      DbType parameterType, object parameterValue)
        {
        ...
        public static DataSet ExecuteDataSet(string connectionString, string commandText, object[] paramList)
        {
            ...
            if (paramList != null)
            {
                SQLiteParameterCollection parms = DeriveParameters(cn, commandText, paramList);
                                             ...
                                             object[] commandParameters)
        {
            ...
            SQLiteParameterCollection coll =
                DeriveParameters(transaction.Connection, cmd.CommandText, commandParameters);
        ...
        public static IDataReader ExecuteReader(SQLiteCommand cmd, string commandText, object[] paramList)
        {
            ...
            cmd.CommandText = commandText;
            SQLiteParameterCollection parms = DeriveParameters(cmd.Connection, commandText, paramList);
        ...
        public static int ExecuteNonQuery(SQLiteTransaction transaction, string commandText, object[] paramList)
        {
            ...
            IDbCommand cmd = transaction.Connection.CreateCommand();
            cmd.CommandText = commandText;
            SQLiteParameterCollection coll = DeriveParameters(transaction.Connection, cmd.CommandText, paramList);
            foreach (SQLiteParameter parm in coll)
                cmd.Parameters.Add(parm);
            if (transaction.Connection.State == ConnectionState.Closed)
                transaction.Connection.Open();
        ...
        public static SQLiteParameterCollection DeriveParameters(SQLiteConnection connection, string commandText,
                                                                 object[] paramList)
        {
        ...
        protected internal static void AssignParameterValues(IDataParameterCollection commandParameters,
                                                             DataRow dataRow)
        {
