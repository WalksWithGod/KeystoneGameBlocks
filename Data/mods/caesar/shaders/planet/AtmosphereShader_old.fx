...
struct VS_OUTPUT {
	...
	float3 viewVec : TEXCOORD1;			// Eye vector 
	float3 normalVec : TEXCOORD2;	
	float3 lightVec : TEXCOORD3;		// Light vector 
};
...
VS_OUTPUT VS(VS_INPUT IN) {
    ...
    OUT.texCoord = IN.texCoord; // todo: this doesnt even seem to be used....
    OUT.lightVec = normalize(IN.rawPos - lightPos);
	return OUT;
}
...
float4 PS(PS_INPUT IN) : COLOR 
{
	...
	return float4(color, saturate(pow(alpha, 4) * dot(normedNormal, IN.lightVec) * 7.0f));
