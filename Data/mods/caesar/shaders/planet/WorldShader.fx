// =============================================================
// World Normal-Mapping Shader
// =============================================================
// http://www.mathematica-users.org/mathematica/images/6/62/specularBumpVertexPixelShader.fx
// https://timewar.svn.codeplex.com/svn/Trunk/TimeWar_v1.5/TimeWar_v1.5/TorqueCore/EngineData/effects/LightingEffect2D.fx
// https://quickstartengine.svn.codeplex.com/svn/framework/test/QuickStartSampleGame/Content/Effects/Terrain.fx
// -------------------------------------------------------------
// Compilation switches
// -------------------------------------------------------------
// #define FORWARD
// 		- only up to 4 point lights supported
//      - directional or spotlights not supported.
//
// #define DEFERRED 
//
...
float4 view 					: VIEWPOS;
//float3 lightPos 				: LIGHTPOINT0_POSITION; // position of primary star
//float3 lightCol 				: LIGHTPOINT0_COLOR;
float3 lightVec 				: LIGHTDIR0_DIRECTION; // = float3 (0,1,0);  LIGHTPOINT0_POSITION; 
float3 lightCol 				: LIGHTDIR0_COLOR;     // range is treated as infinite
...
float4 ps_forward(PS_INPUT IN) : COLOR 
{
	...
	// Parallax mapping, if enabled
		...
		// Using W.Engel...
		float3 reflectVec = 2 * dot(specNormal, lightVecTangentSpace) * specNormal - lightVecTangentSpace;
	...
	lightIntensity = saturate(dot(diffNormal, lightVecTangentSpace)); 
	#endif
	...
	float3 result = saturate(colDiffuse + colSpecular + colEmis); 
    ...
    return float4(result, colDiffMap.a);
...
technique primary 
{
    pass pass0 
	{
		...
		#else
		PixelShader  = compile ps_3_0 ps_forward();
		#endif
    }
}