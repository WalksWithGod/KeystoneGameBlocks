...
float3 v3LightPos;			// The light's current position
...
VertexShaderOutput VertexShaderFunction(VertexShaderInput input)
{
	...
	float3 test=v3FrontColor*fKmESun;
	output.Color1 = float4(test, 1);
	
	test = v3FrontColor * v3InvWavelength * fKrESun;
	output.Color0 = float4(test, 1);
...
{
	float3 ldir = normalize(input.v3Direction);
	float fCos=dot(v3LightDir,ldir);
