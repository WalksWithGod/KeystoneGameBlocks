...
// #define TERRAIN_ATLAS_TEXTURING
...
float4x4 ViewProjection			: VIEWPROJECTION;
...
#elif defined (BILLBOARD_INSTANCING)
	#define InstancesPerBatch 64
	...
	// NOTE: if only Y rotation (UV Rotations) is allowed, w parameter in InstancePositionWS contains Phi in radians.  For complete rotations, we must use seperate InstanceRotations.
	float4 InstancePositionsWS[InstancesPerBatch] : register(c192); 
#else
	float4x4 g_World 			: WORLD;
#endif
...
// -------------------------------------------------------------
	...
	float DEPTH_BIAS = 0.005f; //0.005f; //0.001f; // prevents self shadows when epsilon between casting poly and receiv poly is really low
...
// -------------------------------------------------------------
// Terrain Texture Atlas VARIABLES - "SetShaderParameter" names must match exactly
// -------------------------------------------------------------
#if defined (TERRAIN_ATLAS_TEXTURING)
int AtlasTextureCountX = 4;
int AtlasTextureCountY = 4;
#endif
...
sampler2D sampShadowMap  = sampler_state
{
	...
	sampler2D sampTextureAtlas  = sampler_state
	{
...
// -------------------------------------------------------------
	...
	struct VS_OUTPUT_DEFAULT
	{
		...
		float ClipspaceDepth			: TEXCOORD2;
		float3 ViewDir 					: TEXCOORD5;	// for cell shading and Rim edge lighting
		float4 PositionWS 				: TEXCOORD6;
	};
#endif
...
#elif defined (GEOMETRY_INSTANCING) 
float4x4 GetInstanceMatrix (int instanceIndex)
{
	// w member of return value includes a Y axis rotation value in radians.
	float4 instancePositionPhi = InstancePositionsWS[instanceIndex];
...
#elif defined (BILLBOARD_INSTANCING)
void UnPackFloat(float src, out float r, out float g, out float b)
	{
	  r = frac(src);
	  g = frac(src * 256.0f);
	  b = frac(src * 65536.0f);
 ...
 float4 GetBillboardVertexPositionWS(int instanceIndex, float2 uv) 
{ 
	float4 instancePositionPhi = InstancePositionsWS[instanceIndex];
	
	float3 up;
	UnPackFloat (instancePositionPhi.w, up.x, up.y, up.z); // float3 (0,1,0);
	
    float3 center = instancePositionPhi.xyz;
    float3 eyeVector = center - g_ViewPosition;
    ...
    finalPosition += (uv.x - 0.5f) * sideVector;
    finalPosition += (1.5f - uv.y * 1.5f) * upVector;

    float4 finalPosition4 = float4(finalPosition, 1);

	return finalPosition4;
    ...
    //output.Color = float4( input.Fade, input.Palette, 0, 0 ); 
     
/* 	float4 instancePositionPhi = PositionMS;
	float2 Orientation = float2(0.5f, 0.5f);
	 
    float4 position = float4(instancePositionPhi.xyz, 1.0f); 
    float len = 1.0f; // input.Dimensions.x; 
    float width = 1.0f; // input.Dimensions.y; 
    float offsetLinear = 1.0f; //input.Offset.x; 
    float offsetCamera = 1.0f; //input.Offset.y; 
    float rotation = instancePositionPhi.w; 
     
    float3 ForwardVector = View._m02_m12_m22; 
    float3 RightVector = View._m00_m10_m20; 
    float3 UpVector = View._m01_m11_m21; 
     
//    float3 widthAdjustment = RightVector * ( width * (0.5 - input.Orientation.y ) * cos( rotation ) - len * ( offsetLinear - input.Orientation.x ) * sin( rotation ) ); 
//    float3 lengthAdjustment = UpVector * ( width * (0.5 - input.Orientation.y ) * sin( rotation ) + len * ( offsetLinear - input.Orientation.x ) * cos( rotation ) ); 
//    float3 totalVector = widthAdjustment + lengthAdjustment; 
//    totalVector += offsetCamera * ForwardVector; 
//    position += totalVector; 
 
	float3 widthAdjustment = RightVector * ( width * (0.5 - Orientation.y ) * cos( rotation ) - len * ( offsetLinear - Orientation.x ) * sin( rotation ) ); 
    float3 lengthAdjustment = UpVector * ( width * (0.5 - Orientation.y ) * sin( rotation ) + len * ( offsetLinear - Orientation.x ) * cos( rotation ) ); 
    float3 totalVector = widthAdjustment + lengthAdjustment; 
    totalVector += offsetCamera * ForwardVector; 
    position += float4(totalVector, 0.0f); 
...
//	shadowColor = float3(1.0f, 0.0f, 0.0f);
//	return shadowColor;
...
// fog
//-------------------------------------------------------------------------------------------------
float3 ComputeFogColor(float3 diffuseColor, float dist) 
{
...
// Atlas Texture
//-------------------------------------------------------------------------------------------------
float4 AtlasTextureColor(float2 UV)
{
	...
	float atlasTextureHalfTexelWidth = 0.5f / 512.0f; 
	float atlasTextureHalfTexelHeight = 0.5f / 512.0f;
	
	float2 indices = float2(1, 1);

	// skip if pixel is fully transparent
//	clip(indices.a - 1.0f); // calling clip function can disable early z optimizations on many gpus, but is sometimes necessary for alpha testing
	...
	float wOffset = indices.x * atlasImageUVWidth + atlasTextureHalfTexelWidth;
	float hOffset = indices.y * atlasImageUVHeight + atlasTextureHalfTexelHeight; 
...
//	UV = float2(scaleX, scaleZ);
	
	int offsetX = 0;
	int offsetY = 0;
	
	UV.x = (frac(UV.x) / 4) + offsetX / 4 + 1.0f/1024.0f;
	UV.y = (frac(UV.y) / 4) + offsetY / 4 + 1.0f/1024.0f;

	return tex2D(sampTextureAtlas, UV);
}
#endif
...
#endif
{
	...
	float4x4 instanceMatrix = GetInstanceMatrix (instanceIndex);
...
float4 PS_Shadowed(VS_OUTPUT_SHADOWED IN) : COLOR0
{
	...
	#if defined (TEXTURE_MAPPING_ENABLED)
		#if defined (TERRAIN_ATLAS_TEXTURING)
			diffuseTexture = AtlasTextureColor(IN.UVandDepth.xy);
		#else
	...
	// requires interpolating and normalizing more for each pixel, however, that doesn't mean everything
	// needs to be such as view direction?  And then if a pointlight, what about light direction?  
	// I'm sure it's a case of how accurate do we want to be and what is "good enough" and even unnoticable 
	...
	diffuseColor.rgb = ComputeFogColor (diffuseColor.rgb, IN.UVandDepth.z);
//	diffuseColor.rgb = float3(1, 0, 0);
	return diffuseColor;
}
...
float4 PS_DepthMap(VS_OUT_DEPTH IN) : COLOR0
{
	...
	// using clip.  So ideally you want to only use variant of this shader on meshes that require it.
#if defined (TERRAIN_ATLAS_TEXTURING)
	clip(tex2D(sampTextureAtlas, IN.UV).a - 0.5f);
#else
...
#endif
{
	...
	int instanceIndex = (int)IN.Index;
	//float4 positionWS = float4(InstancePositionsWS[instanceIndex].xyz, 0);
	float4 positionWS = GetBillboardVertexPositionWS (instanceIndex, IN.UV);
	float3 norm = IN.NormalOS; // TODO: Temp value NormalOS is incorrect
...
float4 PS_Default(VS_OUTPUT_DEFAULT IN) : COLOR0
{
	...
	#if defined (TEXTURE_MAPPING_ENABLED)
		#if defined (TERRAIN_ATLAS_TEXTURING)
			diffuseTexture = AtlasTextureColor(IN.UV);	
		#else
			//  TODO: is there a way to avoid 0,0,0,1 being returned from tex2d() if diffuseSampler does not exist?
	...
	// TODO: am I supposed to normalize the ViewDir in vertex shader first and/or not at all there or here?
	// TODO: in general, when to normalize things and when not to.  I do understand that per-pixel-lighting
	// requires interpolating and normalizing more for each pixel, however, that doesn't mean everything needs
	// to be such as view direction?  And then if a pointlight, what about light direction?  Perhaps it's a 
	// case of how accurate do we want to be and what is "good enough" and even unnoticable to do it the more 
	// expensive way	
	float specularLighting = SpecularLighting(normalWS, normalize(IN.ViewDir), g_DirLightDirection);
	...
	// Fog
	diffuseColor.rgb = ComputeFogColor (diffuseColor.rgb, IN.ClipspaceDepth);
//	diffuseColor.rgb = float3(1,0,0);
	return diffuseColor;
}
#endif
...
// -------------------------------------------------------------
#if defined (SHADOW_MAPPING_ENABLED)
	// technique depth_deferred
	// technique shadowed_deferred
	// technique default_deferred
	technique depth
	{
		pass pass0
		{
	...
	technique noshadows
	{
		...
		{
			CULLMODE = CCW;
			// alphablending for particles mostly (not needed for terrain... yet?)
