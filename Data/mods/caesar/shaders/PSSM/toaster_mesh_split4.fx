// -------------------------------------------------------------
/// Mesh Shader - Textured + Normal Mapping + Shadow Mapping
// -------------------------------------------------------------
...
float over_darkening_factor = 60;
...
VS_SHADOWED_OUTPUT VS_Shadowed(VS_SHADOWED_INPUT IN) 
{
	...
	float4 positionWS = mul(IN.vObjectSpacePosition, g_mWorld);
	
	//positionWS = Wind(positionWS);
	OUT.vViewDir = viewPosition - positionWS;
	OUT.vScreenSpacePosition = mul(positionWS, g_mCameraViewProj);
...
{
		// measure cosine of the angle between the 
		// normal at the surface and the direction of the light.
...
// float RampLambert (float3 normal, float3 lightDir)
// {
//	//	requires 1D rampTexture is filled
	// float rampCoord = WrapLambert (normal, lightDir);
...
{
	// clip aborts if value is negative
