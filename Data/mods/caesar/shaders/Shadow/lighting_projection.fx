...
sampler ProjectedMapSampler = sampler_state {
...
float4 PSmain(float2 texCoord: TEXCOORD0, float3 lightVec: TEXCOORD1, float3 smPos: TEXCOORD2, float3 viewVec: TEXCOORD3, float inDepth: TEXCOORD4) : COLOR {
	...
	///float3 Specc		= spec.rgb * specular.rrr;

	// Cubemapping

	float3 newDir		= mul(rotation, nDir);


	float4 Proj		= texCUBE(ProjectedMapSampler, float4(newDir, 1.0));
	float4 C 		= ((diffuse * base) + (specular * spec * base)) * Proj;
