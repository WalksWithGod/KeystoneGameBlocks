...
bool useBlinnSpec < string UIName = "Use Blinn Specular"; > = false;

bool usePhongSpec < string UIName = "Use Phong Specular"; > = false;
bool useAnisotropicSpec < string UIName = "Use Anisotropic Specular"; > = false;

bool useSingleBrdf < string UIName = "Use Single BRDF Diffuse"; > = false;
bool useDualBrdf < string UIName = "Use Dual BRDF Diffuse"; > = false;
bool useSSS < string UIName = "Use Translusence/Sub-Surface Diffuse"; > = false;

bool useFresnel < string UIName = "Use Fresnel Reflection"; > = false;
bool useColorForFresnel < string UIName = "Use Fresnel with Color"; > = false;
bool useReflection < string UIName = "Use Reflection"; > = false;
bool useBlendedAmbient < string UIName = "Reflected/Diffused Ambient"; > = false;
bool useSelfIllumination < string UIName = "Glow/Self Illumination"; > = false;

bool useOffsetMapping < string UIName = "Use Offset Mapping"; > = false;
bool useParallaxOcclusion < string UIName = "Use Parallax Occlusion Mapping"; > = false;
bool useMicroNormal < string UIName = "Use Micro-Normal Texture"; > = false;
...
{  
  ...
  if (useParallaxOcclusion == false) {
    Out.vParallaxOffsetTS = float2(0,0);
        ...
        float3x3 mWorldToTangent = float3x3( Out.vBinormalWS, Out.vTangentWS, Out.vNormalWS );
...
float4 PS(in VS_OUTPUT In) : COLOR0
{
  ...
  float fGlossiness = g_fGlossiness * texSpecular.a;
    ...
    //figure out N (normal)
    texNormal = NormalMapWSTransform3ds (texNormal, Nn, Bn, Tn);  //Function returns unnormalized
    float3	N;
	 
    if (useMicroNormal == true) {
        float4 texMicroNormal = tex2D(samplerMicroNormal, texCoord * g_fMicroNormalScale);
        texMicroNormal = NormalMapWSTransform3ds (texMicroNormal, Nn, Bn, Tn);
