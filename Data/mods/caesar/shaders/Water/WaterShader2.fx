...
VS_OUTPUT VS(VS_INPUT IN) {
	...
	#else
	OUT.oppositeTC = float2(IN.texCoord.x + 0.5f, IN.texCoord.y + time * waveMovementSpeed / 200);		
	#endif
...
float3 CommonPS(PS_INPUT IN) {
	...
	float waveHeight;
		...
		sampledNormal = tex2D(samp2DNormal, texCoord).agbr;
		float4 secondSampling = tex2D(samp2DNormal, oppositeTC).agbr;
		#ifndef PARALLAX_MAPPING
