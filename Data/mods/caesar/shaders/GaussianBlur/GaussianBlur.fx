...
float4 PS(PS_INPUT IN) : COLOR
{
	...
	#endif	
	#if TAPS == 7
		float3x3 positiveSamples;
		float3x3 negativeSamples;
	#endif
	#if TAPS == 9
		float4x3 positiveSamples;
		float4x3 negativeSamples;
	#endif
	
	float3 color = tex2D(sampTexture, IN.centerTap).rgb * centerTapWeight;

	#if TAPS == 5
		for(int i=0; i<4; i++)
			samples[i] = tex2D(sampTexture, IN.taps[i]).rgb;
	#endif
	#if TAPS == 7
		for(int i=0; i<3; i++)
		{
			positiveSamples[i] = tex2D(sampTexture, IN.positiveTaps[i]).rgb;
			negativeSamples[i] = tex2D(sampTexture, IN.negativeTaps[i]).rgb;
		}
	#endif
	#if TAPS == 9
		for(int i=0; i<2; i++)
		{
			positiveSamples[i*2] = tex2D(sampTexture, IN.positiveTaps[i].xy).rgb;
			negativeSamples[i*2] = tex2D(sampTexture, IN.negativeTaps[i].xy).rgb;
			positiveSamples[i*2+1] = tex2D(sampTexture, IN.positiveTaps[i].zw).rgb;
			negativeSamples[i*2+1] = tex2D(sampTexture, IN.negativeTaps[i].zw).rgb;		
		}
	#endif
	
	// The full RGBA version does not work because of a TV3D problem
	// Uncomment and replace above code when fixed?
	/*#if TAPS == 5
		float4x4 samples;
	#endif	
	#if TAPS == 7
