...
{
    /// <summary>
    /// Discovery seems to me to be a way of finding servers on a LAN by listening for and emiting a specific type of broadcast.
    /// This would be useful for private servers on LANS, but I don't think we'll support that at least not for 1.0... perhaps a patch.
    /// </summary>
	...
	internal sealed class NetDiscovery
	{
		...
		)
		{
			...
			string appIdent2 = message.m_buffer.ReadString();
		...
		)
		{
			...
			if (message.m_buffer == null || m_requests == null)
				return null;

			ushort number = message.m_buffer.ReadUInt16();
			...
			resMsg.m_buffer.Write(endPoint);
