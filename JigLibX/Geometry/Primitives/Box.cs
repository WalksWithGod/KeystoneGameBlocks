...
namespace JigLibX.Geometry
{
    ...
    public class Box : Primitive
    {
        ...
        /// MPJ: Origin/Position is based on the back, right, down corner of the box.  The other
        /// corner vertices can be deduced from that as well.  The center of the box
        /// is the sidelengths * 5 * orientation + origin position.
        /// The only question i have left now is, is a transformation applied to this to get it to world coordinates
        /// because i'm assuming that these box coordinates are in the local coordinate system of the entire collision skin
        /// thus in a compound skin, you could have objects at different positions.
        /// This is logical because we know the collision skin itself tracks it's own combined volume.
        /// We also know that CollisionSkin has a ApplyLocalTransform(Transform t) method which itterates it's contained primitives
        /// and multiplies the passed in transform with each of the prim's transforms to compute a new one and then updates the bounding volume
        /// Actually finally i also understand that a collisionSkin manages 3 versions of each primitive!  The local coordinate primitve array
        /// that has all primitives in collisionskin space, then two in world space, one for the previous values and one for the current values
        /// and those reference cloned copies of the box.
        /// </summary>
        ...
        public Box (float positionX, float positionY, float positionZ, float width, float height, float depth) : this(new Vector3( positionX, positionY, positionZ), Matrix.Identity, new Vector3( width, height, depth))
        {
        ...
        {
            // call does the equivalent of extracting the Up/Forward/Right or whatever vector from the matrix using pointers
